...
async function chatWithWaiter({ menu, restaurant, table, lang, sessionState, messages, uiEvent }) {
  ...
  const response = await fetch("https://api.openai.com/v1/responses", {
    ...
    body: JSON.stringify({
      ...
      // UI events never contain a new guest order. Disabling tools here prevents
      // the model from re-proposing the most recent order after Confirm.
      tool_choice: uiEvent ? "none" : "auto",
      text: {
        verbosity: "low",
        format: {
          type: "json_schema",
