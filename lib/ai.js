...
- first_visit: record firstVisit. If true, briefly explain that ordering is completed on this screen, payment is at the register, and Raise Hand calls staff including for allergies. Then ask for drinks. If false, welcome them back and ask for drinks. Move to drinks.
...
- order_confirmed: the proposed order is already confirmed and recorded. Acknowledge it without proposing it again. If dislikesAsked is false, ask about disliked foods, set dislikesAsked true, and move to dislikes. Otherwise thank the guest and continue free ordering.
- dislikes: record stated dislikes (or an empty list when there are none), recommend food avoiding those preferences, and move to food. These are preferences, not safety judgments.
...
async function chatWithWaiter({ menu, restaurant, table, lang, sessionState, messages, uiEvent }) {
  ...
  const response = await fetch("https://api.openai.com/v1/responses", {
    ...
    body: JSON.stringify({
      ...
      reasoning: { effort: process.env.REASONING_EFFORT || "none" },
