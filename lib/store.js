...
function createConfirmedOrder({ table, items, menu }) {
...
function ordersForTable(table) {
  const orders = readJson("orders.json").filter((order) => order.table === table);
...
module.exports = { appendEvent, createConfirmedOrder, ordersForTable, validateItems };
